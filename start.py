...
for subject in subjects:
    ...
    for j in range(2):
        ...
        nogo_epochs_list.append(mne.Epochs(raw,
                    nogo_event_start_frames[j], None,
                    -0.5, 1,
                    baseline=(None,0), 
                    preload=True
                   ))
    ...
    f[0].suptitle('Subject' + subject_idx + ' (Event Onset)',fontsize = 40)
    f[0].set_dpi(1000)
    ...
    #     f= go_evoked.plot(picks=[ch],show=False,time_unit='ms',window_title='Subject' + subject_idx + ' '+ ch + ' (Event Onset)')
    #     plt.title('Subject' + subject_idx + ' '+ ch + ' (Go Event Onset)')
    ...
    #     f=nogo_evoked.plot(picks=[ch],show=False,time_unit='ms',window_title='Subject' + subject_idx + ' '+ ch + ' (Event Onset)')
    #     plt.title('Subject' + subject_idx + ' '+ ch + ' (NoGo Event Onset)')
    #     plt.axvline(x=0,color='k',linestyle='dashed',linewidth='0.75')
    ...
    #     figs = mne.viz.plot_compare_evokeds(go_nogo_evoked,picks=[ch],show=False,title='Subject' + subject_idx + ' '+ ch + ' (Event Onset)',time_unit='ms',show_sensors='lower left',truncate_xaxis=False,truncate_yaxis=False)
